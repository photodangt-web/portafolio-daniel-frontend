  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={form.handleSubmit((values) => save.mutate(values))} className="space-y-6">
        ...
        <ArraySection title="Botones del hero" fields={heroButtons} add={() => heroButtons.append({ label: '', href: '#contact', style: 'outline', openInNewTab: false })}>{(field, index) => <div key={field.id} className="space-y-4 rounded-xl border border-[var(--border)] p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-medium">Botón {index + 1}</p><div className="flex items-center gap-1"><button type="button" disabled={index === 0} onClick={() => heroButtons.move(index, index - 1)} className="rounded-lg p-2 hover:bg-[var(--bg-soft)] disabled:opacity-30" aria-label="Subir botón"><ArrowUp size={15} /></button><button type="button" disabled={index === heroButtons.fields.length - 1} onClick={() => heroButtons.move(index, index + 1)} className="rounded-lg p-2 hover:bg-[var(--bg-soft)] disabled:opacity-30" aria-label="Bajar botón"><ArrowDown size={15} /></button><Remove onClick={() => heroButtons.remove(index)} /></div></div><div className="grid gap-3 sm:grid-cols-[1fr_2fr]"><Input form={form} name={`heroButtons.${index}.label`} label="Texto" /><Input form={form} name={`heroButtons.${index}.href`} label="Destino (#contact o URL)" /></div><div className="grid gap-4 sm:grid-cols-[1fr_auto]"><ButtonStylePicker form={form} name={`heroButtons.${index}.style`} /><label className="mt-6 flex items-center gap-2 text-sm text-[var(--fg-muted)]"><input type="checkbox" {...form.register(`heroButtons.${index}.openInNewTab`)} /> Abrir en nueva ventana</label></div></div>}</ArraySection>
