...
export default function MediaPicker({ value, onChange, label = 'Imagen', accept = 'image' }) {
  ...
  return <div><span className="block text-xs font-medium text-[var(--fg-muted)]">{label}</span><div className="mt-1.5 flex items-center gap-3">{value && <img src={resolveMediaUrl(value)} alt="" className="h-12 w-16 rounded-lg border border-[var(--border)] object-cover" />}<button type="button" onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-lg border border-[var(--border-strong)] px-3 py-2 text-sm hover:bg-[var(--bg-soft)]"><ImagePlus size={16} /> {value ? 'Cambiar' : 'Seleccionar medio'}</button>{value && <button type="button" onClick={() => onChange('')} className="rounded-lg p-2 text-[var(--fg-muted)] hover:bg-[var(--bg-soft)]" aria-label="Quitar medio"><X size={16} /></button>}</div><MediaLibrary open={open} onClose={() => setOpen(false)} accept={kind} onSelect={(item) => { onChange(item.url); setOpen(false) }} /></div>
}
